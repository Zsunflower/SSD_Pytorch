...
class Trainer:
    ...
    def build_model(self):
        if self.cfg.train_cfg.model_name == "SSDModel":
            self.model = SSDModel(self.cfg.n_classes, self.cfg.n_boxes,).to(device)
        elif self.cfg.train_cfg.model_name == "Vgg19BaseSSD":
            self.model = Vgg19BaseSSD(self.cfg.n_classes, self.cfg.n_boxes).to(device)
        else:
            raise Exception("Model name not found!")

        checkpoint_path = os.path.join(
            self.cfg.train_cfg.checkpoint_dir, self.cfg.train_cfg.checkpoint_file
        )
    ...
    def prepare_data(self):
        train_aug = SSDDataAugmentation(
            target_size={"h": self.cfg.img_height, "w": self.cfg.img_width},
        ...
        train_ds = SSDDataset(
            self.cfg.train_cfg.data_dir,
            self.cfg.train_cfg.train_file_path,
            transform=transforms.Compose(
                [train_aug, Transpose(), Normalization(0, 255.0)]
            ),
        )
        eval_ds = SSDDataset(
            self.cfg.train_cfg.data_dir,
        ...
        print(
            "Loaded dataset, train {} samples, eval {} samples".format(
    ...
    def parse_config(self):
        ...
        self.criterion = SSDLoss(
            self.cfg.train_cfg.alpha, self.cfg.train_cfg.neg_pos_ratio
        )
        ...
        anchor_boxes = BoxUtils.generate_anchor_boxes_model(
            predictor_shapes, self.cfg.scales, self.cfg.aspect_ratios
        )
        self.label_encoder = SSDLabelEncoder(
            anchor_boxes,
            self.cfg.nclasses,
            self.cfg.img_height,
            self.cfg.img_width,
        ...
        if os.path.exists(self.cfg.train_cfg.checkpoint_dir) and os.path.isdir(
            self.cfg.train_cfg.checkpoint_dir
        ):
        ...
        for epoch in range(self.cfg.train_cfg.num_epochs):
            ...
            print(
                "Epoch {}, train loss {}, eval loss {}".format(
                    epoch, train_epoch_loss, eval_epoch_loss
                )
            )
            checkpoint_file = "ssd_{}_{:.4f}_{:.4f}.pth".format(
                epoch, train_epoch_loss, eval_epoch_loss
            )
            torch.save(
                self.model.state_dict(),
                os.path.join(self.cfg.train_cfg.checkpoint_dir, checkpoint_file),
            )
