...
class SSDLoss(nn.Module):
    ...
    def ssd_conf_loss(self, y_pred, y_true, pos_mask, neg_mask):
        ...
        y_true_pos_boxes = y_true[pos_mask][
            :, :-4
        ]  # shape (n_positive_boxes, nclasses)
        y_pred_pos_boxes = y_pred[pos_mask][
            :, :-4
        ]  # shape (n_positive_boxes, nclasses)
        y_true_neg_boxes = y_true[neg_mask][
            :, :-4
        ]  # shape (n_negative_boxes, nclasses)
        y_pred_neg_boxes = y_pred[neg_mask][
            :, :-4
        ]  # shape (n_negative_boxes, nclasses)
        pos_conf_loss = torch.sum(
            -y_true_pos_boxes * y_pred_pos_boxes, dim=-1
        )  # shape (n_positive_boxes,)
        neg_conf_loss = torch.sum(
            -y_true_neg_boxes * y_pred_neg_boxes, dim=-1
        )  # shape (n_negative_boxes,)
