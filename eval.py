...
from utils.decode_utils import decode_output, decode_output_decoder
...
class Eval:
    ...
    def build_model(self):
        ...
        checkpoint_path = os.path.join(
            self.cfg.eval_cfg.checkpoint_dir, self.cfg.eval_cfg.checkpoint_file
        )
    ...
    def prepare_data(self):
        eval_aug = SSDDataAugmentation(
            target_size={"h": self.cfg.img_height, "w": self.cfg.img_width}, train=False
        )
        eval_ds = SSDDataset(
            self.cfg.eval_cfg.data_dir,
            self.cfg.eval_cfg.eval_file_path,
            transform=transforms.Compose(
                [eval_aug, Transpose(), Normalization(0, 255.0)]
            ),
        )
        self.eval_loader = torch.utils.data.DataLoader(
            dataset=eval_ds,
            batch_size=self.cfg.eval_cfg.batch_size,
    ...
    def run(self):
        ...
        for sample in self.eval_loader:
            batch_images, batch_labels, batch_filenames = (
                sample["image"],
                sample["objs"],
                sample["filename"],
            )
            ...
            anchor_boxes = BoxUtils.generate_anchor_boxes_model(
                self.model.get_predictor_shapes(x),
                self.cfg.scales,
                self.cfg.aspect_ratios,
            )
            y_pred_decoded = decode_output(
                y_pred,
                anchor_boxes,
                self.cfg.variances,
                self.cfg.img_width,
                self.cfg.img_height,
                self.cfg.nclasses,
                conf_thresh=self.cfg.eval_cfg.threshold,
                iou_thresh=self.cfg.eval_cfg.iou_threshold,
            )

            for (yp, label, filename) in zip(
                y_pred_decoded, batch_labels, batch_filenames
            ):
                ...
                f = open(
                    os.path.join(
                        self.cfg.eval_cfg.groundtruths,
                        os.path.basename(filename).split(".")[0] + ".txt",
                    ),
                    "w",
                )
                for box in label:
                    ...
                    img = cv2.rectangle(
                        img, (box[1], box[2]), (box[3], box[4]), (0, 0, 255), 2
                    )
                f.close()

                f = open(
                    os.path.join(
                        self.cfg.eval_cfg.detections,
                        os.path.basename(filename).split(".")[0] + ".txt",
                    ),
                    "w",
                )
                ...
                cv2.imwrite(
                    os.path.join(
                        self.cfg.eval_cfg.debug_imgs, os.path.basename(filename)
                    ),
                    img,
                )
        print("Eval done!")
        cmd = "python {} -t {} --gtfolder {} --detfolder {} -gtformat xyrb -detformat xyrb --savepath {}".format(
            self.cfg.eval_cfg.cmd_path,
            self.cfg.eval_cfg.threshold,
            os.path.abspath(self.cfg.eval_cfg.groundtruths),
            os.path.abspath(self.cfg.eval_cfg.detections),
            os.path.abspath(self.cfg.eval_cfg.results),
        )
