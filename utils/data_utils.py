    ...
    def __init__(
        ...
        if self.train:
            self.seq_train = iaa.Sequential(
                [
                    iaa.Sometimes(
                        random_brightness["prob"],
                        iaa.Add((random_brightness["low"], random_brightness["high"])),
                    ),
                    iaa.Sometimes(
                        random_contrast["prob"],
                        iaa.LinearContrast(
                            (random_contrast["low"], random_contrast["high"])
                        ),
                    ),
                    iaa.Sometimes(
                        random_saturation["prob"],
                        iaa.MultiplySaturation(
                            (random_saturation["low"], random_saturation["high"])
                        ),
                    ),
                    iaa.Sometimes(
                        random_hue["prob"],
                        iaa.MultiplyHue((random_hue["low"], random_hue["high"])),
                    ),
                    ...
                    iaa.Sometimes(
                        ...
                        iaa.Affine(
                            translate_percent={
                                "x": (
                                    random_translate["low"],
                                    random_translate["high"],
                                ),
                                "y": (
                                    random_translate["low"],
                                    random_translate["high"],
                            ...
                            scale={
                                "x": (random_scale["min"], random_scale["max"]),
            ...
            self.prob = prob
        self.seq_eval = iaa.Resize(
            {"height": target_size["h"], "width": target_size["w"]}
        )
...
def collate_sample(list_samples):
    ...
    return {
        "image": torch.as_tensor(image_batched, dtype=torch.float),
        "objs": objs_batched,
