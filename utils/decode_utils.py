...
def decode_output_decoder(output, traced, nms, conf_thresh=0.5, iou_thresh=0.01):
    ...
    for output_item in output:
        ...
        for class_id in range(1, 1 + 1):
            ...
            nms_filtered = torchvision.ops.nms(
                box_class_id[:, -4:], box_class_id[:, 1], iou_threshold=iou_thresh
            )
...
def decode_output(
    ...
    output[:, :, [-4, -3]] = (
        output[:, :, [-4, -3]] * anchor_boxes[:, :, [-2, -1]]
        + anchor_boxes[:, :, [-4, -3]]
    )
    output[:, :, [-2, -1]] = (
        np.exp(output[:, :, [-2, -1]]) * anchor_boxes[:, :, [-2, -1]]
    )
    ...
    output = np.concatenate(
        [class_max, probs_max, output[:, :, -4:]], axis=-1
...
def nms(boxes, iou_thresh):
    ...
    while len(boxes) > 0:
        ...
        if len(boxes) < 1:
            break
        similarities = BoxUtils.iou_matrix(
            boxes[:, 2:], np.expand_dims(boxes_left[-1][2:], axis=0)
        )  # (N, 1)
